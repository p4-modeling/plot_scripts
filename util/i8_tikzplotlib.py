...
def get_tikz_code(*args, axis_width='10cm', axis_height='5cm',
    ...
    try:
        ...
        # older tikzplotlib has figureheight/width
        print('Maybe old tikzplotlib?')
        code = tikzplotlib.get_tikz_code(*args, 
                                         externalize_tables=True, 
                                         override_externals=True, 
                                         strict=True,
                                         figurewidth=axis_width,
                                         figureheight=axis_height,
                                         **kwargs)
...
# replace some characters that late doesn't like unescaped
def latex_save(string):
